...
//         const res = await fetch("https://team5171pat.veerbajaj11.workers.dev/pat");
//         if (!res.ok) return "";
...
(function () {
    ...
    let isOpen = false;
    let isLoading = false;
    let repoConfig = {}; // Will be populated in init
    let history = [];
    let sessionId = localStorage.getItem('kiwi_chat_session_id') || generateUUID();
    localStorage.setItem('kiwi_chat_session_id', sessionId);
    ...
    async function fetchToken() {
        try {
            const res = await fetch("https://team5171pat.veerbajaj11.workers.dev/pat");
            if (!res.ok) throw new Error("Token server responded with error");
            const data = await res.json();
            return data.token;
        } catch (err) {
            console.error("kiwi-chat: Token fetch failed:", err);
            return "";
        }
    }
    ...
    function createWidget() {
        ...
        panel.innerHTML = `
            <div class="kiwi-chat-header">
                ...
                <div class="kiwi-chat-header-text">
                    <p class="kiwi-chat-header-title">Ask the Kiwi</p>
                    <p class="kiwi-chat-header-sub">AI-powered docs assistant</p>
                </div>
                <button class="kiwi-chat-header-close" id="kiwi-chat-close" aria-label="Close chat">✕</button>
            </div>
            <div class="kiwi-chat-messages" id="kiwi-chat-messages">
            ...
            <div class="kiwi-chat-input-area">
                <textarea class="kiwi-chat-input" id="kiwi-chat-input" placeholder="Ask about the docs…" rows="1"></textarea>
                <button class="kiwi-chat-send" id="kiwi-chat-send">
    ...
    function toggleChat() {
        ...
        if (isOpen) document.getElementById('kiwi-chat-input').focus();
    }
    ...
    function showTyping() {
        const container = document.getElementById('kiwi-chat-messages');
        const div = document.createElement('div');
        div.className = 'kiwi-typing-msg assistant';
        div.id = 'kiwi-typing-indicator';
        div.innerHTML = `<div class="kiwi-chat-msg assistant"><div class="kiwi-typing"><span class="kiwi-typing-dot"></span><span class="kiwi-typing-dot"></span><span class="kiwi-typing-dot"></span></div></div>`;
        container.appendChild(div);
        container.scrollTop = container.scrollHeight;
    }
        ...
        try {
            const res = await fetch(`${endpoint}/api/ask`, {
                ...
                body: JSON.stringify({
                    ...
                    config: repoConfig // This now contains the real token string
                }),
            });
    ...
    // ── Entry Point ──────────────────────────────────────────────────
    async function init() {
        // 1. Wait for the actual token string
        const actualToken = await fetchToken();

        // 2. Build the config using that string
        repoConfig = {
            owner: CONFIG.ai_owner,
            repo: CONFIG.ai_repo,
            branch: CONFIG.ai_branch,
        ...
        // 3. Kick off the UI
        createWidget();
        console.log("Kiwi Chat Initialized.");
    }
